	 ...
	 if(pos == 0)
		 {
			...
			if(buySignal)
				{
				 double sl = lowestLow - 10*point;      // 20日安値の10pips下
				 double tp = ask + (atrValue * ATRMultiplier * 2 / point);  // ATRの2倍
				 if(trade.Buy(lot, Symbol(), ask, sl, tp, "RngBU"))
					 Print("[BUY] RangeBreakUp SL=", sl, " TP=", tp, " ATR=", atrValue);
				}
			else if(sellSignal)
				{
				 double sl = highestHigh + 10*point;     // 20日高値の10pips上
				 double tp = bid - (atrValue * ATRMultiplier * 2 / point);  // ATRの2倍
