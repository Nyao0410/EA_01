...
input int MinTenkanKijunGap = 5;     // 【NEW】転換線と基準線の最小距離(pips) - 緩和
...
void OnTick()
{
    ...
    // 2. 転換線と基準線の距離チェック（オプション）
    if(MinTenkanKijunGap > 0)
    {
    ...
    {
        // シンプルな条件: 前の足が雲下、現在の足が雲上
        bool wasBelow = (close1 <= cloudTop1);  // 雲内または雲下
        bool isAbove = (close0 > cloudTop0);     // 雲上
        
        if(wasBelow && isAbove)
        {
            // 基本条件のみ: 転換線 > 基準線（緩い条件）
            if(tenkan[0] >= kijun[0] || !CheckChikouSpan)
            {
                // 遅行スパンチェック（オプション）
    ...
    if(CloudConfirmBars >= 1)
    {
        // シンプルな条件: 前の足が雲上、現在の足が雲下
        bool wasAbove = (close1 >= cloudBot1);  // 雲内または雲上
        bool isBelow = (close0 < cloudBot0);     // 雲下
